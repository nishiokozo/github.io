...
//-----------------------------------------------------------------------------
{
	...
	//-------------------------------------------------------------------------
	{
		...
		}
		{
			...
			let h	= lab.ch;
			let br	= calc_r(m)/60;
			balls.push({name:"c",p:vec2(1.5, h)	,v:vec2( v, 0) ,a:vec2(0.0,0.0),m:m	,r:br});
...
//				if(0)
				{
					...
					{// 計算
						...
						// 重心を分けず一緒に計算する
						b0.v.x =  vt0.x*m1 -vt1.x*m1	+gx;
						b0.v.y =  vt0.y*m1 -vt1.y*m1	+gy;
						b1.v.x = -vt0.x*m0 +vt1.x*m0	+gx;
						b1.v.y = -vt0.y*m0 +vt1.y*m0	+gy;
...
//			gra.line( gra.sx,0,gra.ex,0 );
		...
		{		
			...
			if ( lab.flgdebug ) gra.symbol_row( ba.name, ba.p.x, ba.p.y);
		}
